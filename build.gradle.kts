...
// Промпты-образцы едут внутри сборки: если рядом с ботом промптов не окажется,
// он выложит их из себя и запустится, а не упадёт на первом же файле.
// Живые промпты лежат рядом в prompts/ и в репозиторий не попадают —
// характер получается личным делом каждого.
tasks.processResources {
    from("prompts-example") { into("prompts") }
}
